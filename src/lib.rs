...
impl Metadata {
  ...
  pub fn parse<R: Read>(reader: &mut R) -> Result<Self, Error> {
    ...
    match reader.read_to_string(&mut s) {
        Ok(_) => {
            Metadata::parse_string(&s).ok_or(
                Error::new(ErrorKind::Other, "parsing failed"))
        }
        Err(why) => {
            Err(why)
        }
      ...
      for (i, name) in names.iter().enumerate() {
          ...
          match self.columns[index].ctype {
              ColumnType::string => {
                  match values[i] {
                      ColumnValue::string { .. } => { }
                      ColumnValue::u32le { .. } => { panic!("Column {} expected string received u32le", names[i]); }
                      ColumnValue::u64le { .. } => { panic!("Column {} expected string received u64le", names[i]); }
                      ColumnValue::null => { }
                  }
              }
              ColumnType::u32le => {
                  match values[i] {
                      ColumnValue::string { .. } => { panic!("Column {} expected u32le received string", names[i]); }
                      ColumnValue::u64le { .. } => { panic!("Column {} expected u32le received u64le", names[i]); }
                      ColumnValue::u32le { .. } => { }
                      ColumnValue::null => { }
                  }
              }
              ColumnType::u64le => {
                  match values[i] {
                      ColumnValue::string { .. } => { panic!("Column {} expected u64le received string", names[i]); }
                      ColumnValue::u32le { .. } => { panic!("Column {} expected u63le received u32le", names[i]); }
                      ColumnValue::u64le { .. } => { }
                      ColumnValue::null => { }
                  }
              }
  ...
  pub fn read<R: Read>(&self, reader: &mut R) -> Row {
      ...
      let mut blobs: Vec<(usize, usize, CompressionType)> = Vec::with_capacity(self.columns.len()); // count only strings?
      let mut result = Row::new(&self.columns);

      for (i, c) in self.columns.iter().enumerate() {
          if header_bits & (1 << i) != 0 { // column is present
              match c.ctype {
                  ColumnType::u32le => {
                      let mut buf = [0; 4];
                      reader.read(&mut buf);
                      ...
                      (buf[2] as usize) << 16 |
                      (buf[3] as usize) << 24;
                      result.push(i, ColumnValue::u32le {
                          v: v as u32
                      });
                  }
                  ColumnType::u64le => {
                      ...
                      (buf[6] as usize) << 48 |
                      (buf[7] as usize) << 56;
                      result.push(i, ColumnValue::u64le {
                          v: v as u64
                      });
                  }
                  ColumnType::string => {
      ...
      for &(i, size, ref compression) in blobs.iter() {
          ...
          result.push(i, ColumnValue::string {
              v: s
          });
      }
...
mod tests {
    ...
    fn it_works() {
        ...
        {
            let r = md.read(&mut f);
            println!("read/1 {:?} {:?} {:?} {:?}", r.getn("a"), r.getn("b"), r.getn("c"), r.getn("d"));
        }
        {
            let r = md.read(&mut f);
            println!("read/2 {:?} {:?} {:?} {:?}", r.geti(0), r.geti(1), r.geti(2), r.geti(3));
        }
        {
            let r = md.read(&mut f);
            println!("read/3 {:?} {:?} {:?} {:?}", r.getn("a"), r.getn("b"), r.getn("c"), r.getn("d"));
        }
    }
    ...
    fn write_works() {
        ...
        let md = Metadata::parse_string(md_string).unwrap();
        let values = [
            ColumnValue::string { v: "hello_world".to_string() },
